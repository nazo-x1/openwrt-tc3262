...
#ifndef CONFIG_MTD_NDM_PARTS
#error SKIP_BAD_BLOCK feature need CONFIG_MTD_NDM_PARTS
#endif
static bool shift_on_bbt;
static bool is_skip_bad_block(struct mtd_info *mtd, int page);
...
static const flashdev_info gen_FlashTable[]= {
	...
	{"MX30LF1G28AD",    0xC2F1, 0x809103, 4, 8,  128, 128, 2048, 112, 0x00444332, 0},
	{"MX30LF2G18AC",    0xC2DA, 0x909506, 5, 8,  256, 128, 2048,  64, 0x00444332, 0},
	{"MX30LF2G28AD",    0xC2DA, 0x909107, 5, 8,  256, 128, 2048, 112, 0x00444332, 0},
	{"MX30LF4G18AC",    0xC2DC, 0x909556, 5, 8,  512, 128, 2048,  64, 0x00444332, 0},

	/* ESMT */
	{"F59L1G81A",       0x92F1, 0x8095FF, 4, 8,  128, 128, 2048,  64, 0x00844333, 0},
	{"F59L1G81MB",      0xC8D1, 0x809540, 4, 8,  128, 128, 2048,  64, 0x00844333, 0},
	{"F59L1G81LB",      0xC8D1, 0x809542, 4, 8,  128, 128, 2048,  64, 0x00844333, 0},
	{"F59L2G81A",       0xC8DA, 0x909544, 5, 8,  256, 128, 2048,  64, 0x00844333, 0},
	{"F59L2G81XA",      0x2CDA, 0x909506, 5, 8,  256, 128, 2048, 112, 0x00844333, 0},
	...
	/* Toshiba */
	{"TC58BVG0S3H",     0x98F1, 0x8015F2, 4, 8,  128, 128, 2048,  64, 0x00844333, 0},
...
static void mtk_nand_readid(struct mtd_info *mtd, struct nand_chip *chip)
{
	...
	chip->select_chip(mtd, NFI_DEFAULT_CS);
	...
	if (nand_devinfo.pagesize == 4096)
		ecc_layout = &nand_oob_128;
	else if (nand_devinfo.pagesize == 2048)
		ecc_layout = &nand_oob_64;
	else if (nand_devinfo.pagesize == 512)
		ecc_layout = &nand_oob_16;

	ecc_layout->eccbytes = nand_devinfo.sparesize - OOB_AVAIL_PER_SECTOR * (nand_devinfo.pagesize / NFI_NAND_SECTOR_SIZE);
	for (i = 0; i < ecc_layout->eccbytes; i++)
		ecc_layout->eccpos[i] = OOB_AVAIL_PER_SECTOR * (nand_devinfo.pagesize / NFI_NAND_SECTOR_SIZE) + i;
}
...
static bool mtk_nand_ready_for_read(struct nand_chip *nand, u32 u4RowAddr, u32 u4ColAddr, bool full)
{
...
{
	bool bRet = false;
	u32 sec_num = 1u << (nand->page_shift - 9);
	u32 colnob = 2, rownob = nand_devinfo.addr_cycle - 2;
...
bool mtk_nand_exec_read_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
	...
	buf = pPageBuf;
	if (mtk_nand_ready_for_read(nand, u4RowAddr, 0, true)) {
...
bool mtk_nand_exec_write_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
	...
	if (mtk_nand_ready_for_write(chip, u4RowAddr, 0, true)) {
...
static int block_remap(struct mtd_info *mtd, int block)
{
	...
	if (get_partition_range(block, chip->phys_erase_shift, &start_blk, &end_blk) < 0)
		return block;
...
#ifdef CONFIG_MTD_NDM_PARTS
	struct nand_chip *chip = mtd->priv;
	uint64_t offs = (uint64_t)page << chip->page_shift;

	if (is_nobbm_partition(offs))
		return false;
#endif
	return true;
}
...
#ifdef CONFIG_MTD_NDM_PARTS
	struct nand_chip *chip = mtd->priv;
	uint64_t offs = (uint64_t)block << chip->phys_erase_shift;

	/* disable skip bad block for storage (or other) */
	if (is_nobbm_partition(offs))
		return block;
#endif
	if (!shift_on_bbt)
...
static int mtk_nand_read_oob_raw(struct mtd_info *mtd, uint8_t * buf, int page_addr, int len)
{
	...
	if (!randomread || !(nand_devinfo.advancedmode & ADV_MODE_RANDOM_READ)) {
		while (len > 0) {
			...
			if (!mtk_nand_ready_for_read(chip, page_addr, col_addr, false)) {
...
static int mtk_nand_write_oob_raw(struct mtd_info *mtd, const uint8_t * buf, int page_addr, int len)
{
	...
	while (len > 0) {
		...
		if (!mtk_nand_ready_for_write(chip, page_addr, col_addr, false)) {
			return -EIO;
		}
...
static int mtk_nand_write_oob_hw(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
	...
	for (i = 0; i < ecc_layout->eccbytes; i++) {
		...
		local_oob_buf[iter] = chip->oob_poi[ecc_layout->eccpos[i]];
	}
...
static int mtk_nand_read_oob_hw(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
	...
	for (i = 0; i < ecc_layout->eccbytes; i++) {
		iter = (i / (spare_per_sector-OOB_AVAIL_PER_SECTOR)) *  spare_per_sector + OOB_AVAIL_PER_SECTOR + i % (spare_per_sector-OOB_AVAIL_PER_SECTOR);
		chip->oob_poi[ecc_layout->eccpos[i]] = local_oob_buf[iter];
	}
...
int mtk_nand_block_bad_hw(struct mtd_info *mtd, loff_t ofs)
{
	...
	unsigned int page_per_block = 1u << (chip->phys_erase_shift - chip->page_shift);
...
static int mtk_nand_block_bad(struct mtd_info *mtd, loff_t ofs)
{
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
...
{
	if (section >= (ecc_layout->eccbytes / 8))
		return -ERANGE;

	oobregion->offset = ecc_layout->eccpos[section * 8];
	oobregion->length = 8;
				  ...
				  struct mtd_oob_region *oobregion)
{
	...
	oobregion->offset = ecc_layout->oobfree[section].offset;
	oobregion->length = ecc_layout->oobfree[section].length;
...
static int mtk_nand_probe(struct platform_device *pdev)
{
	...
	/* PIO mode, buffer must be DWORD aligned */
	...
	mtk_nand_readid(mtd, chip);

	if (nand_scan(mtd, NFI_CS_NUM)) {
